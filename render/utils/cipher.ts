...
import { algo, lib as cryptoLib } from 'crypto-js'
...
export const getLargeFileHash = (file: File): Promise<string> => {
  const hash = algo.SHA256.create();
  const bufferChunkSize = 1024 * 1024;  // 64KB
  let count = 0;
  return new Promise((resolve, reject) => {
    ...
    function dfs(offset) {
      const start = offset
      count = Math.min(start + bufferChunkSize, file.size)
      readChunk(start, count);
    }

    function handleChunk() {
      const arrayBuffer = reader.result as ArrayBuffer;
      const wordArray = cryptoLib.WordArray.create(arrayBuffer);
    ...
    reader.addEventListener('load', handleChunk, false)
